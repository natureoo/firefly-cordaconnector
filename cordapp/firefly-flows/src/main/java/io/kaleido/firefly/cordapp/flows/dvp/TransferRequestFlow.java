...
public class TransferRequestFlow {
    @InitiatingFlow
    @StartableByRPC
    public static class TransferRequest extends CreateFireflyEventFlow<CashMovementState> {
...
//        public LoadRequest(String batchId, UUID groupId, Party settlementBank, Participant payer, Participant payee, AccountInfo fromAccount, AccountInfo toAccount, String customerRefNumber, BigDecimal instructedMVUnit, Currency instructedMVCurrency) {
        public TransferRequest( Party settlementBank, Participant payer, Participant payee, AccountInfo fromAccount, AccountInfo toAccount, String customerRefNumber, BigDecimal instructedMVUnit, Currency instructedMVCurrency) {
...
//            this.batchId = batchId;
        ...
        public static class GENERATING_TRANSACTION extends ProgressTracker.Step {
            public GENERATING_TRANSACTION() {
                super("Generating transaction based on a new Cash Transfer Request");
            }
        }
...
//        setStep(GENERATING_TRANSACTION);
        CashMovementState cashMovementState = new CashMovementState(
                ...
                Instant.now(),
                CashMovementStatus.TRANSFER_REQUEST,
                new UniqueIdentifier()
        );
        Command<CashMovementContract.Commands.CashTransferRequestCmd> txCmd = new Command<>(
                new CashMovementContract.Commands.CashTransferRequestCmd(),
...
//                .collect(Collectors.toList());
    ...
    @InitiatedBy(TransferRequestFlow.TransferRequest.class)
